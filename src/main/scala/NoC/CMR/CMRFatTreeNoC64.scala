...
/**
  ...
  *
  * Geometry is selected by `CMR_FAT_LANE_PROFILE` (`1222` default, or `1248`).
  * IO is 64 cores plus the profile's top parent lanes (2 or 8).  The
  * emitted module name stays `NoC_64nodes` so the 64-core TB can bind
  * without translation.
  *
...
class CMRFatTreeNoC64(
  ...
  require(topLanes == 8 || topLanes == 2,
    s"NoC64 top lanes must be 8 (1248) or 2 (1222), got $topLanes")
...
object CMRFatTreeNoC64Main extends App {
  ...
  private val profile = NoCScaleConfig.fatLaneProfileName
  emitVerilog(
