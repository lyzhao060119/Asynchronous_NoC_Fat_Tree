 ...
 */
class RouterMulticastRequestMaskModule(config: RouterModuleConfig) extends Module {
  val io = IO(new Bundle {
    ...
    val headSelLane = Input(Vec(config.totalPorts, Vec(config.nDirs, UInt(config.laneW.W))))
    val storedLane = Input(Vec(config.totalPorts, Vec(config.nDirs, UInt(config.laneW.W))))
    val holder = Input(Vec(config.totalPorts, UInt(config.holderW.W)))
    val headAllocOk = Input(Vec(config.totalPorts, Bool()))

    val destMask = Output(Vec(config.totalPorts, Vec(config.totalPorts, Bool())))
  })
  ...
  for (i <- 0 until config.totalPorts) {
    ...
    for (o <- 0 until config.totalPorts) {
      bodyGrantedMask(i)(o) := bodyWantedMask(i)(o) && (io.holder(o) === i.U(config.holderW.W))
    }
    ...
    }.elsewhen(io.inValid(i) && !io.isHead(i) && anyDest(io.currentDestVec(i)) && bodyAtomicOk(i)) {
