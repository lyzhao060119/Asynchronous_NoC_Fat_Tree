# NoC 实验设计 V3.0.1
## Novelty-Driven Minimal Evaluation Plan for DATE 2027
...
正式主实验只保留以下 Design ID。V3.0.1 进一步区分：

- **Router-level ASIC target**：真正需要综合 / P&R，用于 PPA、timing、energy；
...
真正需要生成 post-synthesis / post-layout netlist 的核心 Router target 如下。
...
其中 \(A_i\) 来自对应 Router primitive 的实际 synthesis/P&R。
...
**注意**：1024-node 不要求完整 physical netlist 才能进行 resource comparison；V3.0.1 的 PPA 证据来自 primitive-level physical implementation + architecture-level instance accounting。
...
- estimated total Q64 router logic area。

PFAT 若完整高层 P&R 成本过高，最低可接受方案是：
...
其中 256/1024 的 total router area 由已完成物理实现的 Router primitive 按实例数累加，不要求完整网络 P&R；Network performance 由 RTL / timing-aware simulator 获得。
...
这些 primitive 一旦拿到稳定 PPA，后续 64/256/1024 total-area / energy model 才有可信基础。
...
其中 `network_sim` 可以是 parameterized RTL/elaboration target，不要求全部生成 post-layout gate-level netlist。
...
7. 1024-node PPA 使用 physical Router primitive × instance count，不要求整网 P&R；
