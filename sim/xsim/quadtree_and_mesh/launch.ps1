...
$tbCfgFile = Join-Path $tbDir "quadtree_and_mesh_tb_cfg.vh"
...
try {
  ...
  @(
    "// GUI-editable smoke-test configuration for quadtree_and_mesh_tb.",
    "// launch.ps1 rewrites this file to match the selected generated DUT.",
    ('`define QAM_TB_EDGE_N {0}' -f $EdgeN),
    ('`define QAM_TB_N_CORE {0}' -f $nCore),
    ('`define QAM_TB_TOP_LANE {0}' -f $topLane),
    ('`define QAM_TB_HANDSHAKE_TIMEOUT_NS {0}' -f 500000),
    ('`define QAM_TB_GLOBAL_TIMEOUT_NS {0}' -f 8000000)
  ) | Set-Content -Path $tbCfgFile -Encoding Ascii
...
try {
  ...
  if ($Mode -eq "batch") {
    ...
    Write-Host "  $runDir"
    Write-Host "Compiled smoke-test configuration:"
    Write-Host "  EDGE_N=$EdgeN N_CORE=$nCore TOP_LANE=$topLane"
