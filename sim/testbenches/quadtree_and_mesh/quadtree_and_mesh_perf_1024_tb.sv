...
`ifndef PERF_FORCE_FLOW0
`define PERF_FORCE_FLOW0 0
`endif

`ifndef PERF_FLOW0_SRC_Q
`define PERF_FLOW0_SRC_Q 0
`endif

`ifndef PERF_FLOW0_SRC_C
`define PERF_FLOW0_SRC_C 0
`endif

`ifndef PERF_FLOW0_DST_Q
`define PERF_FLOW0_DST_Q 0
`endif

`ifndef PERF_FLOW0_DST_C
`define PERF_FLOW0_DST_C 1
`endif

module quadtree_and_mesh_perf_1024_tb;
  ...
  localparam int GLOBAL_EDGE = EDGE_N * 8;
  ...
  localparam int CFG_FORCE_FLOW0 = `PERF_FORCE_FLOW0;
  localparam int CFG_FLOW0_SRC_Q = `PERF_FLOW0_SRC_Q;
  localparam int CFG_FLOW0_SRC_C = `PERF_FLOW0_SRC_C;
  localparam int CFG_FLOW0_DST_Q = `PERF_FLOW0_DST_Q;
  localparam int CFG_FLOW0_DST_C = `PERF_FLOW0_DST_C;
  ...
  function automatic bit valid_core_coord(input int q, input int c);
    valid_core_coord = ((q >= 0) && (q < N_QUAD) && (c >= 0) && (c < N_CORE));
  endfunction
  ...
  task automatic sample_uniform_rect(
    ...
    rect_x0 = $urandom_range(GLOBAL_EDGE - CFG_RECT_W, 0);
    rect_y0 = $urandom_range(GLOBAL_EDGE - CFG_RECT_H, 0);
  ...
  task automatic sample_overlapping_rect(
    ...
    max_x0 = min2(overlap_gx, GLOBAL_EDGE - CFG_RECT_W);
    min_y0 = max2(0, overlap_gy - CFG_RECT_H + 1);
    max_y0 = min2(overlap_gy, GLOBAL_EDGE - CFG_RECT_H);
  ...
  task automatic encode_flow_packet(input int f);
    int gx;
    int gy;
    int gq;
    int gc;
    ...
    for (gq = 0; gq < N_QUAD; gq = gq + 1) begin
      for (gc = 0; gc < N_CORE; gc = gc + 1) begin
        flow_dest_mask[f][gq][gc] = 1'b0;
      end
    end
  ...
  task automatic apply_flow0_override();
    int f;
    if ((CFG_NUM_FLOWS <= 0) || (CFG_FORCE_FLOW0 == 0)) begin
      return;
    end

    if (!valid_core_coord(CFG_FLOW0_SRC_Q, CFG_FLOW0_SRC_C)) begin
      $fatal(1, "[QAM-PERF] PERF_FLOW0_SRC=(q%0d,c%0d) is out of range", CFG_FLOW0_SRC_Q, CFG_FLOW0_SRC_C);
    end
    if (!valid_core_coord(CFG_FLOW0_DST_Q, CFG_FLOW0_DST_C)) begin
      $fatal(1, "[QAM-PERF] PERF_FLOW0_DST=(q%0d,c%0d) is out of range", CFG_FLOW0_DST_Q, CFG_FLOW0_DST_C);
    end
    if ((CFG_FLOW0_SRC_Q == CFG_FLOW0_DST_Q) && (CFG_FLOW0_SRC_C == CFG_FLOW0_DST_C)) begin
      $fatal(1, "[QAM-PERF] PERF_FLOW0 source and destination must differ");
    end

    for (f = 1; f < CFG_NUM_FLOWS; f = f + 1) begin
      if ((flow_src_q[f] == CFG_FLOW0_SRC_Q) && (flow_src_c[f] == CFG_FLOW0_SRC_C)) begin
        $fatal(1, "[QAM-PERF] PERF_FLOW0 source duplicates flow %0d source; reduce flows or choose another source", f);
      end
    end

    flow_src_q[0] = CFG_FLOW0_SRC_Q;
    flow_src_c[0] = CFG_FLOW0_SRC_C;
    flow_dst_q[0] = CFG_FLOW0_DST_Q;
    flow_dst_c[0] = CFG_FLOW0_DST_C;
    flow_rect_x0[0] = global_x_of_core(CFG_FLOW0_DST_Q, CFG_FLOW0_DST_C);
    flow_rect_y0[0] = global_y_of_core(CFG_FLOW0_DST_Q, CFG_FLOW0_DST_C);
    flow_rect_x1[0] = flow_rect_x0[0];
    flow_rect_y1[0] = flow_rect_y0[0];
    encode_flow_packet(0);
  endtask
  ...
  task automatic build_flow_table();
    ...
    if ((CFG_RECT_W < 1) || (CFG_RECT_W > GLOBAL_EDGE) || (CFG_RECT_H < 1) || (CFG_RECT_H > GLOBAL_EDGE)) begin
      $fatal(1, "[QAM-PERF] PERF_RECT_W/H must be in [1,%0d], got %0d x %0d", GLOBAL_EDGE, CFG_RECT_W, CFG_RECT_H);
    end
    ...
    if (CFG_PATTERN == PERF_PATTERN_OVERLAPPING_MULTICAST) begin
      overlap_gx = $urandom_range(GLOBAL_EDGE - 1, 0);
      overlap_gy = $urandom_range(GLOBAL_EDGE - 1, 0);
    end
    ...
    for (f = 0; f < CFG_NUM_FLOWS; f = f + 1) begin
      ...
      encode_flow_packet(f);
    end

    apply_flow0_override();
  endtask
  ...
  task automatic handle_core_event(
    input int q,
    input int c,
    input logic [FLIT_W-1:0] flit_snapshot,
    input bit count_event,
    input longint unsigned event_ps
  );
    integer flow_idx;
    integer tail_seq;
    integer remaining_tails;
    ...
    if (flow_idx >= 0) begin
      ...
      if (flit_snapshot[27]) begin
        ...
        end else if (flow_head_send_ps[flow_idx][q][c].size() > 0) begin
          send_ps = flow_head_send_ps[flow_idx][q][c].pop_front();
        end else begin
          send_ps = 0;
        end
        if (send_ps != 0) begin
          lat_ps = event_ps - send_ps;
      ...
      if (flit_snapshot[26]) begin
        ...
        if (flow_uses_single_outstanding(flow_idx)) begin
          if (flow_packet_active[flow_idx] && (flow_active_remaining_tails[flow_idx] > 0)) begin
            ...
            if (flow_active_remaining_tails[flow_idx] == 0) begin
              send_ps = flow_active_launch_ps[flow_idx];
              completion_ps = event_ps - send_ps;
        ...
        end else if (flow_tail_seq_q[flow_idx][q][c].size() > 0) begin
          tail_seq = flow_tail_seq_q[flow_idx][q][c].pop_front();
          if (flow_packet_remaining_tails_by_seq[flow_idx].exists(tail_seq)) begin
            remaining_tails = flow_packet_remaining_tails_by_seq[flow_idx][tail_seq] - 1;
            flow_packet_remaining_tails_by_seq[flow_idx][tail_seq] = remaining_tails;
            if (remaining_tails == 0) begin
  ...
  task automatic handle_top_event(input bit count_event);
    if (count_event) begin
      stats_sem.get(1);
  ...
  task automatic service_core_outputs();
    int q;
    int c;
    int dly;
    logic req_snapshot;
    logic [FLIT_W-1:0] flit_snapshot;
    longint unsigned event_ps;
    ...
    forever begin
      ...
      if (reset) begin
        ...
        for (q = 0; q < N_QUAD; q = q + 1) begin
          for (c = 0; c < N_CORE; c = c + 1) begin
            if ((core_out_req[q][c] !== core_out_ack[q][c]) && !core_ack_pending[q][c]) begin
              flit_snapshot = core_out_flit[q][c];
              event_ps = now_ps();
              handle_core_event(q, c, flit_snapshot, measure_active, event_ps);
              core_ack_pending[q][c] = 1'b1;
              dly = core_ack_delay_ns[q][c];
              req_snapshot = core_out_req[q][c];
              fork
                begin : CORE_ACK_DELAY
                  automatic int qq;
                  automatic int cc;
                  automatic int dly_copy;
                  automatic logic req_copy;
                  qq = q;
                  cc = c;
                  dly_copy = dly;
                  req_copy = req_snapshot;
                  #(dly_copy);
                  core_out_ack[qq][cc] = req_copy;
                  core_ack_pending[qq][cc] = 1'b0;
                end
              join_none
            end
          end
  ...
  task automatic service_top_outputs();
    int e;
    int l;
    int dly;
    logic req_snapshot;
    ...
    forever begin
      ...
      if (reset) begin
        ...
        for (e = 0; e < EDGE_N; e = e + 1) begin
          for (l = 0; l < TOP_LANE; l = l + 1) begin
            if ((east_from_req[e][l] !== east_from_ack[e][l]) && !east_from_ack_pending[e][l]) begin
              handle_top_event(measure_active);
              east_from_ack_pending[e][l] = 1'b1;
              dly = edge_ack_delay_ns[e][l];
              req_snapshot = east_from_req[e][l];
              fork
                begin : EAST_ACK_DELAY
                  automatic int ee;
                  automatic int ll;
                  automatic int dly_copy;
                  automatic logic req_copy;
                  ...
                  req_copy = req_snapshot;
                  #(dly_copy);
                  east_from_ack[ee][ll] = req_copy;
                  east_from_ack_pending[ee][ll] = 1'b0;
                end
              join_none
            end

            if ((north_from_req[e][l] !== north_from_ack[e][l]) && !north_from_ack_pending[e][l]) begin
              handle_top_event(measure_active);
              north_from_ack_pending[e][l] = 1'b1;
              dly = edge_ack_delay_ns[e][l];
              req_snapshot = north_from_req[e][l];
              fork
                begin : NORTH_ACK_DELAY
                  automatic int ee;
                  automatic int ll;
                  automatic int dly_copy;
                  automatic logic req_copy;
                  ...
                  req_copy = req_snapshot;
                  #(dly_copy);
                  north_from_ack[ee][ll] = req_copy;
                  north_from_ack_pending[ee][ll] = 1'b0;
                end
              join_none
            end

            if ((west_from_req[e][l] !== west_from_ack[e][l]) && !west_from_ack_pending[e][l]) begin
              handle_top_event(measure_active);
              west_from_ack_pending[e][l] = 1'b1;
              dly = edge_ack_delay_ns[e][l];
              req_snapshot = west_from_req[e][l];
              fork
                begin : WEST_ACK_DELAY
                  automatic int ee;
                  automatic int ll;
                  automatic int dly_copy;
                  automatic logic req_copy;
                  ...
                  req_copy = req_snapshot;
                  #(dly_copy);
                  west_from_ack[ee][ll] = req_copy;
                  west_from_ack_pending[ee][ll] = 1'b0;
                end
              join_none
            end

            if ((south_from_req[e][l] !== south_from_ack[e][l]) && !south_from_ack_pending[e][l]) begin
              handle_top_event(measure_active);
              south_from_ack_pending[e][l] = 1'b1;
              dly = edge_ack_delay_ns[e][l];
              req_snapshot = south_from_req[e][l];
              fork
                begin : SOUTH_ACK_DELAY
                  automatic int ee;
                  automatic int ll;
                  automatic int dly_copy;
                  automatic logic req_copy;
                  ...
                  req_copy = req_snapshot;
                  #(dly_copy);
                  south_from_ack[ee][ll] = req_copy;
                  south_from_ack_pending[ee][ll] = 1'b0;
                end
              join_none
            end
          end
  ...
  initial begin
    ...
    if (CFG_FORCE_FLOW0 != 0) begin
      $display("[QAM-PERF] fixed flow0 override src=(q%0d,c%0d) dst=(q%0d,c%0d)",
        CFG_FLOW0_SRC_Q, CFG_FLOW0_SRC_C, CFG_FLOW0_DST_Q, CFG_FLOW0_DST_C
      );
    end
    for (f = 0; f < CFG_NUM_FLOWS; f = f + 1) begin
      $display("[QAM-PERF] flow=%0d src=(q%0d,c%0d,g%0d,%0d) rect=(%0d,%0d)->(%0d,%0d) copies=%0d id=%0d",
