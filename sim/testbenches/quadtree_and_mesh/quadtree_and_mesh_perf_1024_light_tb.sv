...
module quadtree_and_mesh_perf_1024_light_tb;
  ...
  ps_queue_t flow_head_launch_ps_q [0:MAX_FLOWS-1];
  ps_queue_t flow_tail_launch_ps_q [0:MAX_FLOWS-1];
  ...
  function automatic int flow_match(
    ...
    int idx;
    idx = pktId;
    if ((idx >= 0) && (idx < CFG_NUM_FLOWS) &&
        (q == flow_dst_q[idx]) && (c == flow_dst_c[idx]) &&
        (flow_id[idx] == pktId)) begin
      flow_match = idx;
    end else begin
      flow_match = -1;
    end
  ...
  task automatic configure_pattern_name();
    case (CFG_PATTERN)
      ...
      default: begin
        $fatal(1, "[QAM-PERF] 1024 light tb only supports first-round unicast patterns, got PERF_PATTERN=%0d", CFG_PATTERN);
      end
    endcase
  ...
  task automatic handle_core_event(
    ...
    if (flow_idx >= 0) begin
      ...
      if (flit_snapshot[27]) begin
        if (flow_head_launch_ps_q[flow_idx].size() > 0) begin
          launch_ps = flow_head_launch_ps_q[flow_idx].pop_front();
          lat_ps = event_ps - launch_ps;
      ...
      if (flit_snapshot[26]) begin
        ...
        if (flow_tail_launch_ps_q[flow_idx].size() > 0) begin
          launch_ps = flow_tail_launch_ps_q[flow_idx].pop_front();
          completion_ps = event_ps - launch_ps;
  ...
  task automatic run_flow(input int flow_idx);
    ...
    while (!stop_flows) begin
      ...
      launch_ps = now_ps();
      if (count_flit) begin
        flow_head_launch_ps_q[flow_idx].push_back(launch_ps);
      end
      ...
      launch_ps = now_ps();
      if (count_flit) begin
        flow_tail_launch_ps_q[flow_idx].push_back(launch_ps);
      end
  ...
  task automatic report_summary();
    ...
    pending_packets = 0;
    for (f = 0; f < CFG_NUM_FLOWS; f = f + 1) begin
      pending_heads = pending_heads + flow_head_launch_ps_q[f].size();
      pending_packets = pending_packets + flow_tail_launch_ps_q[f].size();
    end

    $display("");
    $display("[QAM-PERF] ==== 1024 Light Performance Summary ====");
  ...
  initial begin
    ...
    for (f = 0; f < MAX_FLOWS; f = f + 1) begin
      ...
      flow_head_launch_ps_q[f] = {};
      flow_tail_launch_ps_q[f] = {};
    end
    ...
    $display("[QAM-PERF] edge_n=%0d top_lane=%0d handshake_timeout_ns=%0d global_timeout_ns=%0d",
      EDGE_N, TOP_LANE, HANDSHAKE_TIMEOUT_NS, GLOBAL_TIMEOUT_NS
    );
    ...
    if (CFG_FORCE_FLOW0 != 0) begin
      $display("[QAM-PERF] flow0 override enabled src=(q%0d,c%0d) dst=(q%0d,c%0d)",
    ...
    for (f = 0; f < CFG_NUM_FLOWS; f = f + 1) begin
      $display("[QAM-PERF] flow=%0d src=(q%0d,c%0d,g%0d,%0d) dst=(q%0d,c%0d,g%0d,%0d) id=%0d",
        f,
        flow_src_q[f], flow_src_c[f], flow_src_gx[f], flow_src_gy[f],
        flow_dst_q[f], flow_dst_c[f], flow_dst_gx[f], flow_dst_gy[f], flow_id[f]
      );
    end
