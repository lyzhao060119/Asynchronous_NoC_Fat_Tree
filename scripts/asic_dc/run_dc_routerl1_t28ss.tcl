# Synopsys Design Compiler RTL synthesis for async RouterL1 (TSMC 28nm SS CCS).
#
# Run from ~/Asynchronous_Router on a compute node (not login):
#   module load syn
#   bsub -Is dc_shell-t -64 -f scripts/run_dc_routerl1_t28ss.tcl

set PROJECT_DIR "/home/ghy19/Asynchronous_Router"
set RTL_DIR     "$PROJECT_DIR/rtl"
set SCRIPT_DIR  "$PROJECT_DIR/scripts"
set DESIGN_NAME "RouterL1"
set WORK_LIB    "$PROJECT_DIR/work_routerl1"
set REPORT_DIR  "$PROJECT_DIR/reports/RouterL1"
...
# Post-write assertion (shell-level echo in log; DC cannot easily grep)
...
if {$gtech_left > 0} {
  puts "ERROR: RouterL1 netlist still contains GTECH cells"
}

puts "INFO: async RouterL1 DC complete. Results under $REPORT_DIR and $OUTPUT_DIR"
quit
