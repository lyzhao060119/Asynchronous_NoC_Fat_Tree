  ...
  task automatic stop_now(input string marker);
    begin
      if (!failure_fired) begin
        ...
        if (dump_missing_on_fail) begin
          $display("TB_DUMP_MISSING_ON_FAIL writing scoreboard missing-flit CSV before fatal");
          core.dump_missing_expected();
        end
...
`endif
    ...
    if ($test$plusargs("DUMP_MISSING_ON_FAIL")) dump_missing_on_fail = 1'b1;
