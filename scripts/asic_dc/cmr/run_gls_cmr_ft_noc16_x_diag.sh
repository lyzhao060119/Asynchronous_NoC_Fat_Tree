...
$ROOT/sim/tb/tb_cmr_ft_noc16_x_probe.sv
$WORK/sdf_boot.sv
EOF

cd "$WORK"
vcs -full64 -sverilog -timescale=1ns/1ps +neg_tchk +no_notifier -f filelist.f \
  -top tb_noc16_async_axi_bram -top tb_cmr_ft_noc16_x_probe -top sdf_boot \
...
grep -E 'X_FIRST|X_ALSO|X_COUNT|X_GROUP|X_FINISH|X_HEARTBEAT|Error|error' \
  "$LOG/stdout.log" "$LOG/run.log" "$LOG/compile.log" \
  > "$LOG/x_first.log" 2>/dev/null || true
exit "$rc"
