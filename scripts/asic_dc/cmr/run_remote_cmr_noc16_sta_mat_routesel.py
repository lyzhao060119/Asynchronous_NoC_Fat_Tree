#!/usr/bin/env python3
"""Remote per-bit STA of Mat vs RouteSelAnd2 on L2 IPM0."""
import os
import shlex
...
RUN_ID = os.environ.get(
    "CMR_MAT_ROUTESEL_STA_RUN_ID",
    datetime.now().strftime("%Y%m%d_%H%M%S") + "_cmr_mat_routesel_sta",
)
BASELINE = os.environ.get(
    "CMR_NOC16_BASELINE", "20260825_cmr_thin_acg_fifo3_routeseland_dc_01"
)
...
def main():
    ...
    if "OK" not in probe:
        raise RuntimeError("missing DDC " + BASELINE)
    ...
    atomic_put(
        client, sftp,
        REPO / "scripts/asic_dc/cmr/sta_cmr_paired_lib.tcl",
        ROOT + "/scripts/sta/sta_cmr_paired_lib.tcl",
    )
    atomic_put(
        client, sftp,
        REPO / "scripts/asic_dc/cmr/run_sta_cmr_mat_routesel.tcl",
        ROOT + "/scripts/sta/run_sta_cmr_mat_routesel.tcl",
    )
    ...
    body = (
        ...
        "CMR_MAT_ROUTESEL_STA_RUN_ID=%s\n"
        "cd %s\nexec dc_shell-t -64 -f %s/scripts/sta/run_sta_cmr_mat_routesel.tcl\n"
    ...
    if "CMR_MAT_RS_STA_DONE" not in text:
        raise RuntimeError("Mat/RouteSel STA failed")
    print("CMR_MAT_RS_STA_PASS", RUN_ID, flush=True)
