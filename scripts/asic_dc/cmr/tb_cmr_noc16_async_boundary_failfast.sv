...
// CMR-only fail-fast shell around the canonical Ultra event-driven boundary
// checker.  The canonical core remains unchanged and therefore remains usable
// by the existing Ultra simulations.
module tb_cmr_noc16_async_boundary_failfast;
`ifdef CMR_NOC16_TOP2
  localparam integer TOP_LANES = 2;
`else
  localparam integer TOP_LANES = 4;
`endif
  localparam integer NUM_PORTS = 16 + TOP_LANES;

`ifdef CMR_STRUCTURAL_ENDPOINTS
  noc16_async_boundary_core #(
    .STRUCTURAL_ENDPOINTS(1), .ROBUST_DIRECT_HANDSHAKE(0), .TOP_LANES(TOP_LANES)
  ) core();
`else
  noc16_async_boundary_core #(
...
`endif
  ...
  // Body/tail mismatches increment this immediately.  An unmatched Head is
  // held until the following identity flit names pkt_seq; only a bind miss
  // or a leftover Head at dump time increments the counter.
  ...
  initial begin : fail_fast_watchdogs
    ...
    last_progress_ns = $realtime;
    watchdog_armed = 1'b1;
    $display("TB_INFO CMR asynchronous fail-fast armed stall_ns=%0.3f hard_ns=%0.3f",
             stall_timeout_ns, hard_timeout_ns);
    fork
      begin : hard_timeout
        #(hard_timeout_ns);
