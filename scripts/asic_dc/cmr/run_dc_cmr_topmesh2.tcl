# 2x2 CMR TopMesh DC: 4x (2,2) mesh routers, locked hop delay.
# RCU 1xDEL050, matched buf=0, Ackin 1xDEL050. DATE V3 post-synthesis only.
set PROJECT_DIR $::env(CMR_REMOTE_ROOT)
set RUN_ID $::env(CMR_TOPMESH2_RUN_ID)
set RTL_DIR "$PROJECT_DIR/rtl"
set DUT_V "$RTL_DIR/topmesh2/CMRTopMesh.v"
if {[info exists ::env(CMR_TOPMESH2_DUT_V)] && $::env(CMR_TOPMESH2_DUT_V) ne ""} {
  set DUT_V $::env(CMR_TOPMESH2_DUT_V)
}
set REPORT_DIR "$PROJECT_DIR/reports/dc/$RUN_ID"
set OUTPUT_DIR "$PROJECT_DIR/outputs/$RUN_ID"
set WORK_LIB "$PROJECT_DIR/work/dc_topmesh2_$RUN_ID"
file mkdir $REPORT_DIR
file mkdir $OUTPUT_DIR
file mkdir $WORK_LIB

set expected_routers 4
if {[info exists ::env(CMR_EXPECTED_ROUTERS)] && $::env(CMR_EXPECTED_ROUTERS) ne ""} {
  set expected_routers $::env(CMR_EXPECTED_ROUTERS)
}
set expected_ports 40
...
proc cmr_mesh_delay_glob {unit} {
  switch -- $unit {
    ...
    250 { return "DEL250D1*" }
    default {
      puts "CMR_TOPMESH2_DC_FAIL delay_unit $unit"
      exit 2
    }
  }
}
...
elaborate CMRTopMesh -work WORK
current_design CMRTopMesh
uniquify
link
...
puts "CMR_TOPMESH2_LATCH_PRE CLEAR=$clear_pre_count SET=$set_pre_count SR=$sr_pre_count PATH_CLEAR=$path_pre_count PATH_DUAL=$selector_dual_pre_count"
if {$path_pre_count != $expected_path_latches || $selector_dual_pre_count != 0 || $clear_pre_count == 0 || $set_pre_count == 0} {
  puts "CMR_TOPMESH2_DC_FAIL latch_pre_structure path_clear=$path_pre_count expected=$expected_path_latches path_dual=$selector_dual_pre_count clear=$clear_pre_count set=$set_pre_count"
  exit 2
}
...
# Map LanePhaseAdapter / CmrOrReduce. Dont-touch before compile leaves GTECH.
...
set fd [open "$REPORT_DIR/cmr_topmesh2_structure.rpt" w]
...
close $fd
puts "CMR_TOPMESH2_STRUCTURE ROUTER=$router_count FIFO=$async_fifo_count IPM=$ipm_count OPM=$opm_count ADAPTER=$adapter_count TOP=$top_ports MUTEX4=$mutex4_count MUTEX2=$mutex2_count CLEAR=$clear_count SET=$set_count PATH=$path_count CLOSE=$close_count ROUTESEL_AND2=$routesel_and_count DEL050=$del050_rcu ACKIN=$ackin_count ACKIN_UNIT=$opm_ackin_unit_ps ACKIN250=$ackin_del250"

if {$n_gtech > 0 || $n_unmapped > 0} {
  puts "CMR_TOPMESH2_DC_FAIL unmapped gtech=$n_gtech generic=$n_unmapped"
  exit 2
}
if {$router_count != $expected_routers || $async_fifo_count != 0 || $ipm_count != $expected_ports || $opm_count != $expected_ports} {
  puts "CMR_TOPMESH2_DC_FAIL router_or_port_structure router=$router_count fifo=$async_fifo_count ipm=$ipm_count opm=$opm_count"
  exit 2
}
if {$clear_count != $clear_pre_count || $set_count != $set_pre_count || $path_count != $expected_path_latches || $selector_dual_count != 0 || $close_count == 0} {
  puts "CMR_TOPMESH2_DC_FAIL async_storage_structure clear=$clear_count expected_clear=$clear_pre_count set=$set_count expected_set=$set_pre_count path_clear=$path_count expected_path=$expected_path_latches path_dual=$selector_dual_count close=$close_count"
  exit 2
}
if {$handshake_complete_count != 0} {
  puts "CMR_TOPMESH2_DC_FAIL obsolete_handshake_complete actual=$handshake_complete_count expected=0"
  exit 2
}
if {$routesel_and_count != $expected_routesel_and || $routesel_an2_count != $expected_routesel_and} {
  puts "CMR_TOPMESH2_DC_FAIL routesel_and2_structure hier=$routesel_and_count an2=$routesel_an2_count expected=$expected_routesel_and"
  exit 2
}
if {![async_mutex2_drive_mismatch_ok]} {
  puts "CMR_TOPMESH2_DC_FAIL mutex2_nand_drive_mismatch"
  exit 2
}
if {$adapter_count != $expected_adapters} {
  puts "CMR_TOPMESH2_DC_FAIL adapter_count actual=$adapter_count expected=$expected_adapters"
  exit 2
}
if {$top_ports != 0} {
  puts "CMR_TOPMESH2_DC_FAIL unexpected_top_ports actual=$top_ports"
  exit 2
}
if {$rcu_steps == 1 && $rcu_unit_ps == 50} {
  if {$del050_rcu != $expected_ports || $del075_rcu != 0 || $del100_rcu != 0 || $del150_rcu != 0} {
    puts "CMR_TOPMESH2_DC_FAIL rcu_matched_unit DEL050=$del050_rcu expected=$expected_ports DEL075=$del075_rcu DEL100=$del100_rcu DEL150=$del150_rcu"
    exit 2
  }
}
if {$ackin_count != $expected_ports} {
  puts "CMR_TOPMESH2_DC_FAIL opm_ackin_delay actual=$ackin_count expected=$expected_ports unit=$opm_ackin_unit_ps"
  exit 2
}
if {$opm_ackin_unit_ps != 250 && $ackin_del250 != 0} {
  puts "CMR_TOPMESH2_DC_FAIL leftover_ackin_del250 actual=$ackin_del250"
  exit 2
}
...
write -hierarchy -format ddc -output "$OUTPUT_DIR/CMRTopMesh.ddc"
write -hierarchy -format verilog -output "$OUTPUT_DIR/CMRTopMesh_post.v"
write_sdf "$OUTPUT_DIR/CMRTopMesh.sdf"
write_sdc "$OUTPUT_DIR/CMRTopMesh.sdc"
exec sha256sum "$OUTPUT_DIR/CMRTopMesh.ddc" "$OUTPUT_DIR/CMRTopMesh_post.v" "$OUTPUT_DIR/CMRTopMesh.sdf" > "$REPORT_DIR/post_hashes.sha256"
puts "CMR_TOPMESH2_DC_PASS output=$OUTPUT_DIR"
quit
