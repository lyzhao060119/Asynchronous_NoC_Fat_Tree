# CMR fat-tree NoC64 DC entry: 16 L1 + 16 L2 + 16 L3.
...
analyze -format verilog -define ASIC_T28 -work WORK [list \
  ...
  "$RTL_DIR/LanePhaseAdapterDFF.v" \
  "$RTL_DIR/LaneSelector.v" \
...
proc cmr_circular_fifo_count {} {
  ...
  if {[expr {$n % 4}] != 0} {
    puts "PROP_TEMP64_DC_FAIL circular_wcb_alignment wcb=$n"
    exit 2
  }
  return [expr {$n / 4}]
}
...
proc cmr_ft_delay_glob {unit} {
  switch -- $unit {
    ...
    250 { return "DEL250D1*" }
    default {
      puts "PROP_TEMP64_DC_FAIL delay_unit $unit"
      exit 2
    }
  }
}
...
proc cmr_ft_merge_pins {dst extra} {
  if {[sizeof_collection $extra] == 0} {
    return $dst
  }
  if {$dst eq "" || [sizeof_collection $dst] == 0} {
    return $extra
  }
  return [add_to_collection -unique $dst $extra]
}

proc cmr_ft_acklatch_e_pins {} {
  # One enable pin per multi-lane adapter.  Thin 1x1 has zero adapters.
  # LanePhaseAdapterDFF maps the await-arm to LHCNDQD await_latch.E.
  # Legacy LanePhaseAdapter used AckLatch/latch_cell.E.  OffsetLatch is a
  # different RTC and must not be counted.
  set globs [list \
    "*AckLatch*latch_cell/E" \
    "*AwaitAckLatch*await_latch/E" \
    "*LaneAdapterAwaitLatch*await_latch/E" \
    "*await_latch/E"]
  set pins ""
  foreach g $globs {
    set hit [get_pins -hierarchical -quiet -filter "full_name =~ $g"]
    set n [sizeof_collection $hit]
    puts "PROP_TEMP64_ACKLATCH_E_GLOB {$g} count=$n"
    set pins [cmr_ft_merge_pins $pins $hit]
  }
  if {$pins eq ""} {
  ...
  set n_off [sizeof_collection $offset]
  if {$n_off > 0} {
    puts "PROP_TEMP64_ACKLATCH_E_GLOB OffsetLatch excluded=$n_off"
    set pins [remove_from_collection $pins $offset]
  }
  return $pins
}

proc cmr_ft_dump_pin_sample {label coll limit} {
  set n [sizeof_collection $coll]
  puts "PROP_TEMP64_SAMPLE $label count=$n"
  set i 0
...
proc cmr_ft_require_adapter_elaboration {expected_adapters} {
  ...
  set dff [get_cells -hierarchical -quiet -filter {ref_name =~ LanePhaseAdapterDFF*}]
  set sel [get_cells -hierarchical -quiet -filter {ref_name =~ LaneSelector* && ref_name !~ *Celement*}]
  set await_cells [get_cells -hierarchical -quiet -filter {full_name =~ *await_latch && ref_name =~ LHCNDQD*}]
  set await_pins [cmr_ft_acklatch_e_pins]
  set n_dff [sizeof_collection $dff]
  ...
  foreach req {LanePhaseAdapterDFF LaneAdapterAwaitLatch LaneSelector LaneRequestDFF Mux1H} {
    ...
    puts "PROP_TEMP64_ANALYZED_DESIGN $req $n"
  }
  if {$n_dff != $expected_adapters} {
    cmr_ft_dump_pin_sample adapters $dff 8
  ...
  if {$n_await_pins != $expected_adapters} {
    cmr_ft_dump_pin_sample await_e $await_pins 8
    cmr_ft_dump_pin_sample await_cells $await_cells 8
    puts "PROP_TEMP64_DC_FAIL acklatch_e_pre_count actual=$n_await_pins expected=$expected_adapters"
    exit 2
  }
}
...
proc cmr_ft_insert_lane01_bufs {nbuf expected_adapters} {
  ...
  if {$lib_name eq ""} {
    puts "PROP_TEMP64_DC_FAIL lane01_missing_buffd0"
    exit 2
  }
  set pins [cmr_ft_acklatch_e_pins]
  set n [sizeof_collection $pins]
  if {$n != $expected_adapters} {
    puts "PROP_TEMP64_DC_FAIL lane01_acklatch_e_count actual=$n expected=$expected_adapters"
    exit 2
  }
  set pin_names {}
  ...
  for {set stage 1} {$stage <= $nbuf} {incr stage} {
    foreach pin_name $pin_names {
      ...
      if {[sizeof_collection $pin] != 1} {
        puts "PROP_TEMP64_DC_FAIL lane01_buf_pin $pin_name"
        exit 2
      }
      if {[catch {insert_buffer $pin $lib_name -new_cell_names lane01_acklatch_e_buf_s${stage}} err]} {
        puts "PROP_TEMP64_DC_FAIL lane01_buf_insert stage=$stage pin=$pin_name error=$err"
        exit 2
      }
      incr inserted
    }
  ...
  if {$inserted != $expected || [sizeof_collection $cells] != $expected} {
    puts "PROP_TEMP64_DC_FAIL lane01_buf_count inserts=$inserted cells=[sizeof_collection $cells] expected=$expected"
    exit 2
  }
...
proc cmr_ft_insert_cfifo_hs02 {} {
  ...
  if {$circular != $expected_fifos} {
    puts "PROP_TEMP64_DC_FAIL cfifo_hs02_fifo_count actual=$circular expected=$expected_fifos"
    exit 2
  }
  set targets [get_pins -hierarchical -quiet -filter {full_name =~ *read_counter/U10/A1}]
  if {[sizeof_collection $targets] != $circular} {
    puts "PROP_TEMP64_DC_FAIL cfifo_hs02_bind actual=[sizeof_collection $targets] expected=$circular"
    exit 2
  }
  set lib_name [cmr_ft_lib_cell BUFFD0BWP12T30P140]
  if {$lib_name eq ""} {
    puts "PROP_TEMP64_DC_FAIL cfifo_hs02_missing_buffd0"
    exit 2
  }
  set inserted 0
  for {set stage 1} {$stage <= 3} {incr stage} {
    foreach_in_collection target $targets {
      if {[catch {insert_buffer $target $lib_name -new_cell_names cfifo_hs02_ack_counter_buf_s${stage}} err]} {
        puts "PROP_TEMP64_DC_FAIL cfifo_hs02_insert stage=$stage pin=[get_object_name $target] error=$err"
        exit 2
      }
      incr inserted
    }
  ...
  if {$inserted != $expected || [sizeof_collection $cells] != $expected} {
    puts "PROP_TEMP64_DC_FAIL cfifo_hs02_count inserts=$inserted cells=[sizeof_collection $cells] expected=$expected"
    exit 2
  }
  set_dont_touch $cells true
...
proc cmr_ft_insert_cfifo_rd01 {} {
  ...
  if {[sizeof_collection $reqouts] != $circular} {
    puts "PROP_TEMP64_DC_FAIL cfifo_rd01_reqout_count actual=[sizeof_collection $reqouts] expected=$circular"
    exit 2
  }
  ...
  if {$n_named == $n4} {
    ...
    puts "PROP_TEMP64_DC_FAIL cfifo_rd01_xor_bind named_u2=$n_named reqout_xor=$n_mapped expected=$n4 or $n2"
    exit 2
  }
  ...
  set lib_name [cmr_ft_lib_cell BUFFD0BWP12T30P140]
  if {$lib_name eq ""} {
    puts "PROP_TEMP64_DC_FAIL cfifo_rd01_missing_buffd0"
    exit 2
  }
  set stages 16
  set inserted 0
  for {set stage 1} {$stage <= $stages} {incr stage} {
    foreach_in_collection target $targets {
      if {[catch {insert_buffer $target $lib_name -new_cell_names cfifo_rd01_reqout_buf_s${stage}} err]} {
        puts "PROP_TEMP64_DC_FAIL cfifo_rd01_insert stage=$stage pin=[get_object_name $target] error=$err"
        exit 2
      }
      incr inserted
    }
  ...
  if {$inserted != $expected || [sizeof_collection $cells] != $expected} {
    puts "PROP_TEMP64_DC_FAIL cfifo_rd01_buffer_count inserts=$inserted cells=[sizeof_collection $cells] expected=$expected"
    exit 2
  }
  set_dont_touch $cells true
...
puts "PROP_TEMP64_LATCH_PRE CLEAR=$clear_pre_count SET=$set_pre_count SR=$sr_pre_count PATH_CLEAR=$path_pre_count PATH_DUAL=$selector_dual_pre_count"
if {$path_pre_count != $expected_path_latches || $selector_dual_pre_count != 0 || $clear_pre_count == 0 || $set_pre_count == 0} {
  puts "PROP_TEMP64_DC_FAIL latch_pre_structure path_clear=$path_pre_count expected=$expected_path_latches path_dual=$selector_dual_pre_count clear=$clear_pre_count set=$set_pre_count"
  exit 2
}
...
set otherpath_del050 [sizeof_collection [get_cells -hierarchical -quiet -filter {ref_name =~ DEL050D1* && full_name =~ *OtherPathDelay*}]]
set expected_otherpath [expr {4 * $expected_ports}]
...
foreach {key val} [list \
  ...
  OTHERPATH_DEL050_COUNT $otherpath_del050 \
] {
  puts $fd [format "%s=%s" $key $val]
}
close $fd
puts [format "PROP_TEMP64_STRUCTURE ROUTER=%s FIFO=%s ASYNC_FIFO=%s CIRCULAR_FIFO=%s IPM=%s OPM=%s ADAPTER=%s ACKLATCH_E=%s LANE01_BUF=%s stages=%s MUTEX4=%s MUTEX2=%s CLEAR=%s SET=%s SR=%s CLOSE=%s COMPLETE=%s ROUTESEL_AND2=%s ROUTESEL_AN2D0=%s DEL050=%s ACKIN=%s ACKIN_UNIT=%s ACKIN250=%s HS02=%s RD01=%s FIFO_OUTREQ_DEL150=%s OTHERPATH_DEL050=%s" \
  $router_count $fifo_count $async_fifo_count $circular_fifo_count $ipm_count $opm_count $adapter_count $acklatch_e_count $lane01_buf $lane01_stages $mutex4_count $mutex2_count $clear_count $set_count $sr_count $close_count $handshake_complete_count $routesel_and_count $routesel_an2_count $del050_rcu $ackin_count $opm_ackin_unit_ps $ackin_del250 $cfifo_hs02_buf $cfifo_rd01_buf $fifo_del150 $otherpath_del050]

if {$n_gtech > 0 || $n_unmapped > 0} {
  puts "PROP_TEMP64_DC_FAIL unmapped gtech=$n_gtech generic=$n_unmapped"
  exit 2
}
if {$router_count != $expected_routers || $fifo_count != $expected_fifos || $ipm_count != $expected_ports || $opm_count != $expected_ports} {
  puts "PROP_TEMP64_DC_FAIL router_or_fifo_structure router=$router_count fifo=$fifo_count ipm=$ipm_count opm=$opm_count"
  exit 2
}
if {$clear_count != $clear_pre_count || $set_count != $set_pre_count || $path_count != $expected_path_latches || $selector_dual_count != 0 || $close_count == 0} {
  puts "PROP_TEMP64_DC_FAIL async_storage_structure clear=$clear_count expected_clear=$clear_pre_count set=$set_count expected_set=$set_pre_count path_clear=$path_count expected_path=$expected_path_latches path_dual=$selector_dual_count close=$close_count"
  exit 2
}
if {$handshake_complete_count != 0} {
  puts "PROP_TEMP64_DC_FAIL obsolete_handshake_complete actual=$handshake_complete_count expected=0"
  exit 2
}
if {$routesel_and_count != $expected_routesel_and || $routesel_an2_count != $expected_routesel_and} {
  puts "PROP_TEMP64_DC_FAIL routesel_and2_structure hier=$routesel_and_count an2=$routesel_an2_count expected=$expected_routesel_and"
  exit 2
}
if {![async_mutex2_drive_mismatch_ok]} {
  puts "PROP_TEMP64_DC_FAIL mutex2_nand_drive_mismatch"
  exit 2
}
if {$adapter_count != $expected_adapters} {
  puts "PROP_TEMP64_DC_FAIL adapter_count actual=$adapter_count expected=$expected_adapters"
  exit 2
}
if {$acklatch_e_count != $expected_adapters} {
  cmr_ft_dump_pin_sample await_e_post [cmr_ft_acklatch_e_pins] 8
  puts "PROP_TEMP64_DC_FAIL acklatch_e_count actual=$acklatch_e_count expected=$expected_adapters"
  exit 2
}
puts "PROP_TEMP64_ACKLATCH_E_COUNT actual=$acklatch_e_count expected=$expected_adapters"
set expected_lane01_buf [expr {$expected_adapters * $lane01_stages}]
if {$lane01_buf != $expected_lane01_buf} {
  puts "PROP_TEMP64_DC_FAIL lane01_acklatch_buf actual=$lane01_buf expected=$expected_lane01_buf stages=$lane01_stages"
  exit 2
}
if {$rcu_steps == 1 && $rcu_unit_ps == 50} {
  if {$del050_rcu != $expected_ports || $del075_rcu != 0 || $del100_rcu != 0 || $del150_rcu != 0} {
    puts "PROP_TEMP64_DC_FAIL rcu_matched_unit DEL050=$del050_rcu expected=$expected_ports DEL075=$del075_rcu DEL100=$del100_rcu DEL150=$del150_rcu"
    exit 2
  }
}
if {$rcu_steps == 1 && $rcu_unit_ps == 150} {
  if {$del150_rcu != $expected_ports || $del050_rcu != 0 || $del075_rcu != 0 || $del100_rcu != 0} {
    puts "PROP_TEMP64_DC_FAIL rcu_matched_unit DEL150=$del150_rcu expected=$expected_ports DEL050=$del050_rcu DEL075=$del075_rcu DEL100=$del100_rcu"
    exit 2
  }
}
if {$otherpath_del050 != $expected_otherpath} {
  puts "PROP_TEMP64_DC_FAIL otherpath_delay DEL050=$otherpath_del050 expected=$expected_otherpath"
  exit 2
}
if {$ackin_count != $expected_ports} {
  puts "PROP_TEMP64_DC_FAIL opm_ackin_delay actual=$ackin_count expected=$expected_ports unit=$opm_ackin_unit_ps"
  exit 2
}
if {$opm_ackin_unit_ps != 250 && $ackin_del250 != 0} {
  puts "PROP_TEMP64_DC_FAIL leftover_ackin_del250 actual=$ackin_del250"
  exit 2
}
if {$expected_fifos == 0} {
  if {$circular_fifo_count != 0 || $async_fifo_count != 0 || $cfifo_hs02_buf != 0 || $cfifo_rd01_buf != 0 || $fifo_del150 != 0} {
    puts "PROP_TEMP64_DC_FAIL unexpected_fifo_on_bypass circular=$circular_fifo_count async=$async_fifo_count HS02=$cfifo_hs02_buf RD01=$cfifo_rd01_buf FIFO_DEL150=$fifo_del150"
    exit 2
  }
  puts "PROP_TEMP64_BYPASS_INTERLEVEL_FIFO fifos=0"
  ...
  if {$cfifo_hs02_buf != $expected_hs02} {
    puts "PROP_TEMP64_DC_FAIL cfifo_hs02_buf actual=$cfifo_hs02_buf expected=$expected_hs02"
    exit 2
  }
  if {$cfifo_rd01_buf != $CMR_FT_RD01_EXPECTED} {
    puts "PROP_TEMP64_DC_FAIL cfifo_rd01_buf actual=$cfifo_rd01_buf expected=$CMR_FT_RD01_EXPECTED"
    exit 2
  }
  if {$fifo_del150 != 0} {
    puts "PROP_TEMP64_DC_FAIL unexpected_acg_outreq_on_circular actual=$fifo_del150"
    exit 2
  }
} elseif {$async_fifo_count == $expected_fifos} {
  set expected_outreq [expr {$expected_fifos * 3}]
  if {$circular_fifo_count != 0} {
    puts "PROP_TEMP64_DC_FAIL mixed_fifo_kind circular=$circular_fifo_count async=$async_fifo_count"
    exit 2
  }
  if {$fifo_del150 != $expected_outreq} {
    puts "PROP_TEMP64_DC_FAIL fifo_outreq_del150 actual=$fifo_del150 expected=$expected_outreq"
    exit 2
  }
  if {$cfifo_hs02_buf != 0 || $cfifo_rd01_buf != 0} {
    puts "PROP_TEMP64_DC_FAIL unexpected_cfifo_eco_on_acg HS02=$cfifo_hs02_buf RD01=$cfifo_rd01_buf"
    exit 2
  }
  puts "CMR_FIFO_01 outReqDelay DEL150=$fifo_del150 stages=3 fifos=$async_fifo_count"
} else {
  puts "PROP_TEMP64_DC_FAIL fifo_kind circular=$circular_fifo_count async=$async_fifo_count expected=$expected_fifos"
  exit 2
}
