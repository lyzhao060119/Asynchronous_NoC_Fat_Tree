...
EXPECTED_SYNC = {
    ...
    "router_l1_c1_p4": {"selectors": 4, "ports": 8, "mutex_widths": set()},
...
def parse_router(text: str) -> dict:
    ...
    # LaneSelector is emitted as a BlackBox, so its internal CMRMutexN does
    # not appear in CMRRouter.v.  Its LANES parameter is the mutex width that
    # will be elaborated from the companion resource RTL during DC.
    mutex_widths.update(
        int(match)
        for match in re.findall(
            r"LaneSelector\s*#\s*\(\s*\.\s*LANES\s*\(\s*(\d+)\s*\)",
            text,
        )
    )
