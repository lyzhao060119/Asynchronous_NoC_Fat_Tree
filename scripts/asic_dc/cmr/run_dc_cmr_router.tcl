...
proc cmr_router_acklatch_e_pins {} {
  ...
  foreach g [list \
    ...
    if {[sizeof_collection $hit] > 0} {
      if {[sizeof_collection $pins] == 0} {
        set pins $hit
      } else {
        set pins [add_to_collection -unique $pins $hit]
      }
    }
  }
...
if {[info exists ::env(CMR_DONT_TOUCH_ADAPTERS)] && $::env(CMR_DONT_TOUCH_ADAPTERS) eq "1"} {
  # Mapping must precede preservation: locking an unmapped RTL design leaves
  # GTECH cells in the GLS netlist.  Preserve the mapped instances instead.
  set adapter_cells [get_cells -hierarchical -quiet -filter {ref_name =~ LanePhaseAdapter*}]
  set mutex_cells [get_cells -hierarchical -quiet -filter {ref_name =~ CMRMutexN* || ref_name =~ CMRFlatArbiter* || ref_name =~ CMRTAC*}]
  if {[sizeof_collection $adapter_cells] > 0} { set_dont_touch $adapter_cells true }
  if {[sizeof_collection $mutex_cells] > 0} { set_dont_touch $mutex_cells true }
  puts "INFO: post_map_dont_touch adapters=[sizeof_collection $adapter_cells] mutex_tac=[sizeof_collection $mutex_cells]"
}
...
if {$otherpath_del050 != $expected_otherpath} {
  puts "CMR_DC_FAIL otherpath_del050 actual=$otherpath_del050 expected=$expected_otherpath"
  exit 2
}
