...
proc cmr_ft_acklatch_e_pins {} {
  ...
  foreach g $globs {
    ...
    puts "CMR_NOC64_ACKLATCH_E_GLOB {$g} count=$n"
  ...
  if {$n_off > 0} {
    puts "CMR_NOC64_ACKLATCH_E_GLOB OffsetLatch excluded=$n_off"
...
proc cmr_ft_dump_pin_sample {label coll limit} {
  ...
  puts "CMR_NOC64_SAMPLE $label count=$n"
  set i 0
...
proc cmr_ft_require_adapter_elaboration {expected_adapters} {
  ...
  puts "CMR_NOC64_ADAPTER_ELAB DFF=$n_dff SELECTOR=$n_sel AWAIT_LATCH=$n_await_cells AWAIT_E=$n_await_pins expected=$expected_adapters"
  foreach req {LanePhaseAdapterDFF LaneAdapterAwaitLatch LaneSelector LaneRequestDFF Mux1H} {
    ...
    puts "CMR_NOC64_ANALYZED_DESIGN $req $n"
  }
  if {$n_dff != $expected_adapters} {
    cmr_ft_dump_pin_sample adapters $dff 8
    puts "CMR_NOC64_DC_FAIL adapter_elab_count actual=$n_dff expected=$expected_adapters"
    exit 2
  }
  if {$n_sel != $expected_adapters} {
    cmr_ft_dump_pin_sample selectors $sel 8
    puts "CMR_NOC64_DC_FAIL selector_elab_count actual=$n_sel expected=$expected_adapters"
    exit 2
  }
  if {$n_await_pins != $expected_adapters} {
    ...
    puts "CMR_NOC64_DC_FAIL acklatch_e_pre_count actual=$n_await_pins expected=$expected_adapters"
    exit 2
  }
}
...
close $fd
puts [format "CMR_NOC64_STRUCTURE ROUTER=%s FIFO=%s ASYNC_FIFO=%s CIRCULAR_FIFO=%s IPM=%s OPM=%s ADAPTER=%s ACKLATCH_E=%s LANE01_BUF=%s stages=%s MUTEX4=%s MUTEX2=%s CLEAR=%s SET=%s SR=%s CLOSE=%s COMPLETE=%s ROUTESEL_AND2=%s ROUTESEL_AN2D0=%s DEL050=%s ACKIN=%s ACKIN_UNIT=%s ACKIN250=%s HS02=%s RD01=%s FIFO_OUTREQ_DEL150=%s OTHERPATH_DEL050=%s" \
...
if {$adapter_count != $expected_adapters} {
  puts "CMR_NOC64_DC_FAIL adapter_count actual=$adapter_count expected=$expected_adapters"
  exit 2
}
...
puts "CMR_NOC64_ACKLATCH_E_COUNT actual=$acklatch_e_count expected=$expected_adapters"
...
if {$otherpath_del050 != $expected_otherpath} {
  puts "CMR_NOC64_DC_FAIL otherpath_delay DEL050=$otherpath_del050 expected=$expected_otherpath"
  exit 2
}
