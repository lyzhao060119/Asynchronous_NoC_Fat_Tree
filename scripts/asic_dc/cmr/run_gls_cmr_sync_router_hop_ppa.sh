...
case "$KIND" in
  ...
  sync_fat_1x2) VCS_DEFINE="+define+GEOM_C1_P2" ;;
  sync_fat_1x4) VCS_DEFINE="+define+GEOM_C1_P4" ;;
  sync_pfat_2x4) VCS_DEFINE="+define+GEOM_C2_P4" ;;
  sync_pfat_4x8) VCS_DEFINE="+define+GEOM_C4_P8" ;;
...
TB_NAME=${CMR_HOP_TB_NAME:-tb_sync_cmr_router_hop_ppa.sv}
case "$TB_NAME" in
  tb_sync_cmr_router_hop_ppa.sv|tb_sync_cmr_router_multi_lane_agg.sv) ;;
  *) echo "Unsupported sync CMR_HOP_TB_NAME: $TB_NAME" >&2; exit 2 ;;
esac
...
cp "$TB" "$WORK/tb_sync_cmr_router_hop_ppa.sv"
cp "$BIND"/*.vi "$WORK/"
...
if [ "$TB_NAME" = "tb_sync_cmr_router_multi_lane_agg.sv" ]; then
