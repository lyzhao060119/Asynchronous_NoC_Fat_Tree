...
netlists.  Unset CMR_NOC16_NETLIST_RUN_ID.  Async Mesh64 GLS requires V3
materialized cases (exponential header starts, contiguous packet flits);
the legacy Bernoulli 3-flit fallback has been removed.  Functional GLS is
opt-in via CMR_MESH64_SKIP_FUNC=0.
"""
...
# Async NoC64 experiments use only V3 canonical traces: exponential header
# arrivals and contiguous packet flits.  The legacy 3-flit Bernoulli cases
# are deliberately not an available fallback for Mesh64 GLS.
ALLOW_V3 = True
...
def case_local_path(name: str) -> Path:
    if V3_CASE_DIR is not None:
        candidate = V3_CASE_DIR / (name + ".case")
        if candidate.is_file():
            return candidate
    raise SystemExit("missing V3 Mesh64 case directory or case " + name)
...
def _check_generated(generated: Path) -> Path:
    ...
    matched_params = "#(.DelayUnitPs(%s), .DelayValue(%s))" % (
        RCU_UNIT_PS,
        RCU_STEPS,
    )
    matched_ok = (
        ("DelayElement %s MatchedDelay" % matched_params) in text
        or ("DelayElement %s BundlingSignal_MatchedDelay" % matched_params) in text
    )
    ackin = "DelayElement #(.DelayUnitPs(%s), .DelayValue(1)) AckinDelay" % ACKIN_UNIT_PS
    if not matched_ok:
        raise SystemExit("mesh64 MatchedDelay is not %sxDEL%s" % (RCU_STEPS, RCU_UNIT_PS))
    if ackin not in text:
        raise SystemExit("mesh64 AckinDelay is not 1xDEL%s" % ACKIN_UNIT_PS)
    ...
    if reopen in text:
        print("LOCAL_EMIT latch_reopen=%s" % reopen, flush=True)
    else:
        print("LOCAL_EMIT latch_reopen=absent", flush=True)
...
def generate_rtl() -> Path:
    ...
    env["CMR_MESH_RCU_MATCHED_DELAY_UNIT_PS"] = str(RCU_UNIT_PS)
...
def main():
    ...
    if V3_CASE_DIR is None or not V3_CASE_DIR.is_dir():
        raise SystemExit("async Mesh64 GLS requires CMR_NOC64_V3_CASE_DIR or CMR_MESH64_V3_CASE_DIR")
    if SKIP_FUNC:
    ...
    if int(ACKIN_UNIT_PS) != 50 or int(RCU_STEPS) < 1 or int(RCU_UNIT_PS) not in (50, 75, 100, 150):
        raise SystemExit(
            "mesh64 delay recipe invalid: RCU requires positive steps and DEL050/075/100/150; Ackin must be DEL050, got rcu=%sx%s ackin=%s"
