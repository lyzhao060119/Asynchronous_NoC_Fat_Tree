...
if {[info exists ::env(CMR_EXPECTED_TOP)] && $::env(CMR_EXPECTED_TOP) ne ""} {
  set expected_top $::env(CMR_EXPECTED_TOP)
}
...
set top_input_count [sizeof_collection [get_ports -quiet io_top_input_*_hs_valid]]
set top_output_count [sizeof_collection [get_ports -quiet io_top_output_*_hs_valid]]
...
puts $fd "TOP_INPUT_COUNT=$top_input_count"
puts $fd "TOP_OUTPUT_COUNT=$top_output_count"
puts $fd "WNS=$wns"
puts $fd "WHS=$whs"
close $fd
puts "CMR_SYNC64_STRUCTURE ROUTER=$router_count IPM=$ipm_count OPM=$opm_count TOP_IN=$top_input_count TOP_OUT=$top_output_count MUTEX=$mutex_count DELAY=$delay_count ADAPTER=$adapter_count FIFO=$fifo_count SELECTOR=$selector_count LATCH=$latch_count WNS=$wns WHS=$whs"
...
if {$top_input_count != $expected_top || $top_output_count != $expected_top} {
  puts "CMR_SYNC64_DC_FAIL top_port_structure in=$top_input_count out=$top_output_count expected=$expected_top"
  exit 2
}
