...
def check_generated(design_id: str, generated: Path) -> Path:
    ...
    if owner in ("THIN64", "PROP64", "PFAT64", "FM64"):
        require_emit_locked_delays(text, label=design_id, ackin_unit_ps=50,
                                   rcu_unit_ps=row["delay_recipe"]["rcu_unit_ps"])
    ...
    # Current CMR RTL instantiates the DFF-based adapter.  Accept the legacy
    # spelling too, but count only elaborated adapter instances (not modules).
    adapter_count = len(
        re.findall(
            r"^\s+LanePhaseAdapter(?:DFF)?\s*#\s*\(\s*\.\s*LANES\s*\(\s*\d+\s*\)",
            text,
            re.M,
        )
    )
