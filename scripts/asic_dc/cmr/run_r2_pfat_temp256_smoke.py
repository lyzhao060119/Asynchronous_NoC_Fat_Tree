#!/usr/bin/env python3
"""Round-2 PFAT_temp256 smoke: KEY-256 + low-load UR on frozen hier netlist."""
...
NODES = 256
KIND = "pfat_temp"
BUNDLE = HERE / "generated_cases" / "20260915_pfat_temp256_smoke_202701"
CASE_DIR = BUNDLE / "cases"
TRACE_DIR = BUNDLE / "traces"
STATE_DIR = HERE / "results" / "r2_pfat_temp256_smoke"
...
SMOKE_CASES = (
    "KEY-256_n256_s%d_corners_PFAT_temp256_top0" % SEED,
    "TOPO-UR_n256_s%d_m5_PFAT_temp256_top0" % SEED,
)
...
def checked_run_id(value: str) -> str:
    ...
    refuse_overwrite(value, action="pfat_temp256_smoke_gls")
    return value
...
def materialize() -> None:
    ...
    header = {
        ...
        "nodes": NODES,
        "seed": SEED,
        "seed_set_id": "paper256_pfat_smoke",
    ...
    for name in SMOKE_CASES:
        ...
        client, submitted = remote_run_failover(
            ...
            % (
                ...
                shlex.quote(log),
                shlex.quote(log),
                shlex.quote("pfat256smoke_" + name.split("_")[0]),
...
def main() -> int:
    ...
    run_id = checked_run_id(args.run_id or ("%s_cmr_pfat_temp256_smoke" % stamp()))
    ...
    if args.stage in ("submit", "all"):
        ...
        try:
            ...
            client.close()
        print("PFAT_SMOKE_RUN", run_id, "netlist", NETLIST, flush=True)
    return 0
