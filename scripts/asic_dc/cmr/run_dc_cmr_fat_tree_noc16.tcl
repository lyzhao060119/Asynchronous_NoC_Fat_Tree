# CMR fat-tree NoC16 DC entry: four L1 1->2 routers, one L2 2->4 router
# and sixteen depth-3 inter-level asynchronous FIFOs.
set PROJECT_DIR $::env(CMR_REMOTE_ROOT)
set RUN_ID $::env(CMR_NOC16_RUN_ID)
set RTL_DIR "$PROJECT_DIR/rtl"
set DUT_V "$RTL_DIR/NoC_16nodes.v"
if {[info exists ::env(CMR_NOC16_DUT_V)] && $::env(CMR_NOC16_DUT_V) ne ""} {
  set DUT_V $::env(CMR_NOC16_DUT_V)
}
set REPORT_DIR "$PROJECT_DIR/reports/dc/$RUN_ID"
set OUTPUT_DIR "$PROJECT_DIR/outputs/$RUN_ID"
set WORK_LIB "$PROJECT_DIR/work/dc_ft_noc16_$RUN_ID"
file mkdir $REPORT_DIR
file mkdir $OUTPUT_DIR
file mkdir $WORK_LIB

set expected_routers 5
set expected_fifos 16
if {[info exists ::env(CMR_BYPASS_INTERLEVEL_FIFO)] && $::env(CMR_BYPASS_INTERLEVEL_FIFO) eq "1"} {
  set expected_fifos 0
}
set expected_ports 36
...
set expected_adapters [expr {[info exists ::env(CMR_EXPECTED_ADAPTERS)] ? $::env(CMR_EXPECTED_ADAPTERS) : 64}]
set rcu_unit_ps 50
...
proc cmr_ft_matched_leaves {unit} {
  switch -- $unit {
    50  { set glob "DEL050D1*" }
    75  { set glob "DEL075D1*" }
    100 { set glob "DEL100D1*" }
    150 { set glob "DEL150D1*" }
    default {
      puts "CMR_NOC16_DC_FAIL rcu_matched_unit $unit"
      exit 2
    }
  }
...
proc cmr_ft_insert_lane01_bufs {nbuf expected_adapters} {
  ...
  if {$lib_name eq ""} {
    puts "CMR_NOC16_DC_FAIL lane01_missing_buffd0"
    exit 2
  }
  set pins [cmr_ft_acklatch_e_pins]
  set n [sizeof_collection $pins]
  if {$n != $expected_adapters} {
    puts "CMR_NOC16_DC_FAIL lane01_acklatch_e_count actual=$n expected=$expected_adapters"
    exit 2
  }
  set pin_names {}
  ...
  for {set stage 1} {$stage <= $nbuf} {incr stage} {
    foreach pin_name $pin_names {
      ...
      if {[sizeof_collection $pin] != 1} {
        puts "CMR_NOC16_DC_FAIL lane01_buf_pin $pin_name"
        exit 2
      }
      if {[catch {insert_buffer $pin $lib_name -new_cell_names lane01_acklatch_e_buf_s${stage}} err]} {
        puts "CMR_NOC16_DC_FAIL lane01_buf_insert stage=$stage pin=$pin_name error=$err"
        exit 2
      }
      incr inserted
    }
  ...
  if {$inserted != $expected || [sizeof_collection $cells] != $expected} {
    puts "CMR_NOC16_DC_FAIL lane01_buf_count inserts=$inserted cells=[sizeof_collection $cells] expected=$expected"
    exit 2
  }
...
proc cmr_ft_insert_cfifo_hs02 {} {
  ...
  if {$circular != $expected_fifos} {
    puts "CMR_NOC16_DC_FAIL cfifo_hs02_fifo_count actual=$circular expected=$expected_fifos"
    exit 2
  }
...
proc cmr_ft_insert_cfifo_rd01 {} {
  ...
  if {[sizeof_collection $reqouts] != $circular} {
    puts "CMR_NOC16_DC_FAIL cfifo_rd01_reqout_count actual=[sizeof_collection $reqouts] expected=$circular"
    exit 2
  }
  ...
  if {$n_named == $n4} {
    ...
    puts "CMR_NOC16_DC_FAIL cfifo_rd01_xor_bind named_u2=$n_named reqout_xor=$n_mapped expected=$n4 or $n2"
    exit 2
  }
...
puts "CMR_NOC16_LATCH_PRE CLEAR=$clear_pre_count SET=$set_pre_count SR=$sr_pre_count PATH_CLEAR=$path_pre_count PATH_DUAL=$selector_dual_pre_count"
if {$path_pre_count != $expected_path_latches || $selector_dual_pre_count != 0 || $clear_pre_count == 0 || $set_pre_count == 0} {
  puts "CMR_NOC16_DC_FAIL latch_pre_structure path_clear=$path_pre_count expected=$expected_path_latches path_dual=$selector_dual_pre_count clear=$clear_pre_count set=$set_pre_count"
  exit 2
}
...
set ackin_del250 [sizeof_collection [get_cells -hierarchical -quiet -filter {ref_name =~ DEL250D1* && full_name =~ *AckinDelay*}]]
set fifo_del150 [sizeof_collection [get_cells -hierarchical -quiet -filter {ref_name =~ DEL150D1* && full_name =~ *outReqDelay*}]]

set fd [open "$REPORT_DIR/cmr_fat_tree_noc16_structure.rpt" w]
...
if {$router_count != $expected_routers || $fifo_count != $expected_fifos || $ipm_count != $expected_ports || $opm_count != $expected_ports} {
  puts "CMR_NOC16_DC_FAIL router_or_fifo_structure router=$router_count fifo=$fifo_count ipm=$ipm_count opm=$opm_count"
  exit 2
}
if {$clear_count != $clear_pre_count || $set_count != $set_pre_count || $path_count != $expected_path_latches || $selector_dual_count != 0 || $close_count == 0} {
  puts "CMR_NOC16_DC_FAIL async_storage_structure clear=$clear_count expected_clear=$clear_pre_count set=$set_count expected_set=$set_pre_count path_clear=$path_count expected_path=$expected_path_latches path_dual=$selector_dual_count close=$close_count"
  exit 2
}
...
if {$routesel_and_count != $expected_routesel_and || $routesel_an2_count != $expected_routesel_and} {
  puts "CMR_NOC16_DC_FAIL routesel_and2_structure hier=$routesel_and_count an2=$routesel_an2_count expected=$expected_routesel_and"
  exit 2
}
if {![async_mutex2_drive_mismatch_ok]} {
  puts "CMR_NOC16_DC_FAIL mutex2_nand_drive_mismatch"
  exit 2
}
if {$acklatch_e_count != $expected_adapters} {
  puts "CMR_NOC16_DC_FAIL acklatch_e_count actual=$acklatch_e_count expected=$expected_adapters"
  exit 2
}
set expected_lane01_buf [expr {$expected_adapters * $lane01_stages}]
if {$lane01_buf != $expected_lane01_buf} {
  puts "CMR_NOC16_DC_FAIL lane01_acklatch_buf actual=$lane01_buf expected=$expected_lane01_buf stages=$lane01_stages"
  exit 2
}
if {$rcu_steps == 1 && $rcu_unit_ps == 50} {
  if {$del050_rcu != $expected_ports || $del075_rcu != 0 || $del100_rcu != 0 || $del150_rcu != 0} {
    puts "CMR_NOC16_DC_FAIL rcu_matched_unit DEL050=$del050_rcu expected=$expected_ports DEL075=$del075_rcu DEL100=$del100_rcu DEL150=$del150_rcu"
    exit 2
  }
}
if {$ackin_del250 != $expected_ports} {
  puts "CMR_NOC16_DC_FAIL opm_ackin_del250 actual=$ackin_del250 expected=$expected_ports"
  exit 2
}
if {$expected_fifos == 0} {
  if {$circular_fifo_count != 0 || $async_fifo_count != 0 || $cfifo_hs02_buf != 0 || $cfifo_rd01_buf != 0 || $fifo_del150 != 0} {
    puts "CMR_NOC16_DC_FAIL unexpected_fifo_on_bypass circular=$circular_fifo_count async=$async_fifo_count HS02=$cfifo_hs02_buf RD01=$cfifo_rd01_buf FIFO_DEL150=$fifo_del150"
    exit 2
  }
  puts "CMR_NOC16_BYPASS_INTERLEVEL_FIFO fifos=0"
  ...
  if {$cfifo_hs02_buf != $expected_hs02} {
    puts "CMR_NOC16_DC_FAIL cfifo_hs02_buf actual=$cfifo_hs02_buf expected=$expected_hs02"
    exit 2
  }
  if {$cfifo_rd01_buf != $CMR_FT_RD01_EXPECTED} {
    puts "CMR_NOC16_DC_FAIL cfifo_rd01_buf actual=$cfifo_rd01_buf expected=$CMR_FT_RD01_EXPECTED"
    exit 2
  }
  if {$fifo_del150 != 0} {
    puts "CMR_NOC16_DC_FAIL unexpected_acg_outreq_on_circular actual=$fifo_del150"
    exit 2
  }
} elseif {$async_fifo_count == $expected_fifos} {
  set expected_outreq [expr {$expected_fifos * 3}]
  if {$circular_fifo_count != 0} {
    puts "CMR_NOC16_DC_FAIL mixed_fifo_kind circular=$circular_fifo_count async=$async_fifo_count"
    exit 2
  }
  if {$fifo_del150 != $expected_outreq} {
    puts "CMR_NOC16_DC_FAIL fifo_outreq_del150 actual=$fifo_del150 expected=$expected_outreq"
    exit 2
  }
  if {$cfifo_hs02_buf != 0 || $cfifo_rd01_buf != 0} {
    puts "CMR_NOC16_DC_FAIL unexpected_cfifo_eco_on_acg HS02=$cfifo_hs02_buf RD01=$cfifo_rd01_buf"
    exit 2
  }
  puts "CMR_FIFO_01 outReqDelay DEL150=$fifo_del150 stages=3 fifos=$async_fifo_count"
} else {
  puts "CMR_NOC16_DC_FAIL fifo_kind circular=$circular_fifo_count async=$async_fifo_count expected=$expected_fifos"
  exit 2
}
