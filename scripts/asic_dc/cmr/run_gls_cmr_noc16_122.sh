#!/bin/bash
# CMR NoC16 Fat 1-2-2 GLS: MODE=func (no SDF) or MODE=sdf (MAXIMUM).
set -euo pipefail
...
TB_DEFINE="+define+CMR_NOC16_TOP2"
SDF_SCOPE="tb_cmr_noc16_async_boundary_failfast.core.g_behavioral_noc_122.noc.dut"
...
test -s "$CASE_FILE"
test -s "$NETLIST_RAW"
test -s "$ROOT/sim/tb/async_noc16_port_adapter_top2.sv"
test -s "$ROOT/sim/tb/tb_noc16_async_boundary.sv"
test -s "$ROOT/sim/tb/tb_cmr_noc16_async_boundary_failfast.sv"
...
sha256sum "$CASE_FILE" "$NETLIST_RAW" "$LIB" \
  "$ROOT/sim/tb/async_noc16_port_adapter_top2.sv" \
  "$ROOT/sim/tb/tb_noc16_async_boundary.sv" \
  "$ROOT/sim/tb/tb_cmr_noc16_async_boundary_failfast.sv" | tee "$LOG/input_hashes.log"
echo "CMR_NOC16_122_GLS mode=$MODE rx_capture=$RX_CAPTURE_NS scope=$SDF_SCOPE" | tee -a "$LOG/input_hashes.log"
...
if [[ "$MODE" == "func" ]]; then
  ...
  if [[ -z "$PY" ]]; then
    echo "CMR_NOC16_122_GLS_FAIL no Python interpreter for functional netlist patch" >&2
    exit 2
  fi
  ...
  if ! grep -q 'q0 = ~(req0 & q1)' "$NETLIST" || ! grep -q 'assign #(1.0)' "$NETLIST"; then
    echo "CMR_NOC16_122_GLS_FAIL functional Mutex/Delay patch incomplete" >&2
    exit 2
  fi
...
vcs -full64 -sverilog -timescale=1ns/1ps $TIMING_ARGS $TB_DEFINE -f filelist.f \
  -top tb_cmr_noc16_async_boundary_failfast $TOP_EXTRA \
...
if [[ "$INJECT_MAX_RATE" == "1" ]]; then
  ...
  echo "CMR_NOC16_122_GLS_FAIL invalid INJECT_MAX_RATE=$INJECT_MAX_RATE" >&2
  exit 2
fi
