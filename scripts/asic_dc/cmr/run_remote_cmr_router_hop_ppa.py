...
def ssh_chunk_put(client, local: Path, remote: str) -> str:
    """Hash-verified atomic upload over SSH exec, avoiding unstable SFTP writes."""
    data = local.read_bytes().replace(b"\r\n", b"\n")
    digest = hashlib.sha256(data).hexdigest()
    existing = remote_run(client, "sha256sum %s 2>/dev/null" % shlex.quote(remote))
    if digest in existing:
        print("UPLOAD_SKIP", remote, flush=True)
        return digest
    temporary = "%s.upload.%d" % (remote, os.getpid())
    remote_run(client, ": > %s" % shlex.quote(temporary))
    ...
    observed = remote_run(
        client,
        "base64 -d %s > %s.decoded && sha256sum %s.decoded && mv -f %s.decoded %s && rm -f %s"
        % (
            shlex.quote(temporary), shlex.quote(temporary), shlex.quote(temporary),
            shlex.quote(temporary), shlex.quote(remote), shlex.quote(temporary),
        ),
    )
    if digest not in observed:
        raise RuntimeError("SSH upload hash mismatch for %s: %s" % (remote, observed))
    print("UPLOAD_OK", remote, flush=True)
    return digest
...
def parse_power(path: Path) -> dict:
    ...
    # PrimeTime's time-based report normally prints Internal, Switching and
    # Leakage totals, but no line named "Total Dynamic Power".  Dynamic power
    # is Internal + Switching, equivalently Total - Leakage.  Do not label the
    # Net Switching-only fallback as total dynamic power.
    if result["total_power_w"] is not None and result["leakage_power_w"] is not None:
        result["dynamic_power_w"] = result["total_power_w"] - result["leakage_power_w"]
    return result
...
def main() -> None:
    ...
    try:
        ...
        for local, remote in uploads:
            ssh_chunk_put(client, local, remote)
        checked(
            client,
