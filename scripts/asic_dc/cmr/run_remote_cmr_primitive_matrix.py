...
from cmr_primitive_geometries import DEFAULT_HOP_KINDS, HOP_PPA_RUN_ID, all_primitives, lookup
...
def delay_env(base: dict[str, str] | None = None) -> dict[str, str]:
    ...
    env.setdefault("C1_HOST", "192.168.2.9")
    ...
    env["CMR_MESH_RCU_MATCHED_DELAY_UNIT_PS"] = "50"
    ...
    env.pop("CMR_NOC16_NETLIST_RUN_ID", None)
...
def hop_ppa() -> None:
    ...
    env["CMR_HOP_KINDS"] = os.environ.get(
        "CMR_HOP_KINDS", ",".join(DEFAULT_HOP_KINDS)
    )
...
def write_registry() -> None:
    ...
    if calibration.is_file():
        target = cal_dir / ("%s.json" % HOP_PPA_RUN_ID)
