...
# Delay recipe is supplied by the frozen run environment.  Baseline uses
# RCU 1xDEL050/Ackin 1xDEL050; mitigation runs may select RCU DEL150.
...
set expected_phase_selectors [expr {4 * $expected_ports}]
...
set rps_pre [get_cells -hierarchical -quiet -filter {ref_name =~ ReadPhaseSelector*}]
# Anchor on the instance leaf name.  A bare *CorrectionToggle / *OtherPathDelay
# glob also matches generate children under Toggle / DelayElement (ResetInv,
# ClearResetToggle, DelayUnit_chain_*), inflating the count by 2-4x.
set correction_pre [get_cells -hierarchical -quiet -filter {full_name =~ */CorrectionToggle}]
set otherpath_pre [get_cells -hierarchical -quiet -filter {full_name =~ */OtherPathDelay}]
# OPMSelector PathLatch is the legal Fig. 6 route state.  Only a PathLatch
# under the Fig. 8 ReadInterface_* instances is the obsolete selector.
set legacy_path_pre [get_cells -hierarchical -quiet -filter {full_name =~ */ReadInterface_*/PathLatch*}]
set rps_pre_count [sizeof_collection $rps_pre]
set correction_pre_count [sizeof_collection $correction_pre]
set otherpath_pre_count [sizeof_collection $otherpath_pre]
set legacy_path_pre_count [sizeof_collection $legacy_path_pre]
puts "CMR_MESH64_SELECTOR_PRE RPS=$rps_pre_count CORRECTION=$correction_pre_count OTHERPATH=$otherpath_pre_count LEGACY_PATH=$legacy_path_pre_count CLEAR=$clear_pre_count SET=$set_pre_count SR=$sr_pre_count"
if {$rps_pre_count != $expected_phase_selectors || $correction_pre_count != $expected_phase_selectors || $otherpath_pre_count != $expected_phase_selectors || $legacy_path_pre_count != 0 || $clear_pre_count == 0 || $set_pre_count == 0} {
  puts "CMR_MESH64_DC_FAIL selector_pre_structure rps=$rps_pre_count correction=$correction_pre_count otherpath=$otherpath_pre_count expected=$expected_phase_selectors legacy_path=$legacy_path_pre_count clear=$clear_pre_count set=$set_pre_count"
  exit 2
}
...
set rps_count [sizeof_collection [get_cells -hierarchical -quiet -filter {ref_name =~ ReadPhaseSelector*}]]
set correction_count [sizeof_collection [get_cells -hierarchical -quiet -filter {full_name =~ */CorrectionToggle}]]
set legacy_path_count [sizeof_collection [get_cells -hierarchical -quiet -filter {full_name =~ */ReadInterface_*/PathLatch*}]]
...
set rcu_matched_count [sizeof_collection [cmr_mesh_matched_leaves $rcu_unit_ps]]
set expected_rcu_matched_count [expr {$expected_ports * $rcu_steps}]
...
puts $fd "READ_PHASE_SELECTOR_COUNT=$rps_count"
puts $fd "CORRECTION_TOGGLE_COUNT=$correction_count"
puts $fd "LEGACY_PATH_LATCH_COUNT=$legacy_path_count"
...
puts $fd "RCU_MATCHED_DELAY_COUNT=$rcu_matched_count"
puts $fd "RCU_MATCHED_DELAY_EXPECTED_COUNT=$expected_rcu_matched_count"
...
puts $fd "OTHERPATH_DEL050_COUNT=$otherpath_del050"
close $fd
puts "CMR_MESH64_STRUCTURE ROUTER=$router_count FIFO=$async_fifo_count IPM=$ipm_count OPM=$opm_count ADAPTER=$adapter_count TOP=$top_ports RPS=$rps_count CORRECTION=$correction_count LEGACY_PATH=$legacy_path_count MUTEX4=$mutex4_count MUTEX2=$mutex2_count CLEAR=$clear_count SET=$set_count CLOSE=$close_count ROUTESEL_AND2=$routesel_and_count RCU_UNIT=$rcu_unit_ps RCU_STEPS=$rcu_steps RCU_DELAY_COUNT=$rcu_matched_count RCU_DELAY_EXPECTED=$expected_rcu_matched_count DEL050=$del050_rcu DEL150=$del150_rcu ACKIN=$ackin_count ACKIN_UNIT=$opm_ackin_unit_ps ACKIN250=$ackin_del250 LATCH_REOPEN=$latch_reopen_count GRANT_HOLD=$grant_hold_buf OTHERPATH_DEL050=$otherpath_del050"
...
if {$clear_count != $clear_pre_count || $set_count != $set_pre_count || $rps_count != $expected_phase_selectors || $correction_count != $expected_phase_selectors || $legacy_path_count != 0 || $close_count == 0} {
  puts "CMR_MESH64_DC_FAIL selector_post_structure clear=$clear_count expected_clear=$clear_pre_count set=$set_count expected_set=$set_pre_count rps=$rps_count correction=$correction_count expected=$expected_phase_selectors legacy_path=$legacy_path_count close=$close_count"
  exit 2
}
...
if {$rcu_unit_ps != 50 && $rcu_unit_ps != 75 && $rcu_unit_ps != 100 && $rcu_unit_ps != 150} {
  puts "CMR_MESH64_DC_FAIL unsupported_rcu_unit unit=$rcu_unit_ps"
  exit 2
}
set rcu_other_count [expr {$del050_rcu + $del075_rcu + $del100_rcu + $del150_rcu - $rcu_matched_count}]
if {$rcu_matched_count != $expected_rcu_matched_count || $rcu_other_count != 0} {
  puts "CMR_MESH64_DC_FAIL rcu_matched_recipe unit=$rcu_unit_ps steps=$rcu_steps actual=$rcu_matched_count expected=$expected_rcu_matched_count other=$rcu_other_count DEL050=$del050_rcu DEL075=$del075_rcu DEL100=$del100_rcu DEL150=$del150_rcu"
  exit 2
}
if {$otherpath_del050 != $expected_otherpath} {
  puts "CMR_MESH64_DC_FAIL otherpath_delay DEL050=$otherpath_del050 expected=$expected_otherpath"
  exit 2
}
